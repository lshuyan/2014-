...
- (void)setEdit:(BOOL)edit
{
    ...
    if ([_delegateSetSkin respondsToSelector:@selector(scrollViewSetSkin:didChangedEdit:)]) {
        [_delegateSetSkin scrollViewSetSkin:self didChangedEdit:_edit];
    }
}
...
- (void)createAddtionButton
{
    ...
    // 添加事件
    [_viewSetSkinItemAdd addTarget:self action:@selector(onTouchItem:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)onTouchItem:(UIViewSetSkinItem *)viewSetSkinItem
{
    if (_edit) {
        return;
    }
    
    if (_viewSetSkinItemAdd==viewSetSkinItem) {
        // TODO:添加操作
        ...
        _skinInfo.skinIndex = fileIndex;
        _skinInfo.isAppInternal = isInternal;
...
- (void)addSkinImageWithPath:(NSString *)path animated:(BOOL)animated;
{
    ...
    {
        NSString *skinName = [NSString stringWithFormat:@"%@%lu", LocalizedString(@"pifu"), (unsigned long)_arrViewItem.count - _arrSysSkinName.count];
    ...
    [viewSetSkinItem addTarget:self action:@selector(onTouchItem:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)removeItemAtIndex:(NSInteger)index
{
    ...
    for (UIViewSetSkinItem *viewSetSkin in _arrViewItem) {
        ...
        if (viewSetSkin.selected && viewSetSkin.tag==index) {
            [SVProgressHUD showErrorWithStatus:LocalizedString(@"tupianzhengzaishiyong_bunengshanchu")];
            return;
        }
    }
