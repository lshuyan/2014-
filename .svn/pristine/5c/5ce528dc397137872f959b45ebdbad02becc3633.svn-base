...
- (void)setBookmarkIsExist:(BOOL)bookmarkIsExist
{
    _bookmarkIsExist = bookmarkIsExist;
    if (bookmarkIsExist) {
    ...
    _btnBookmark.selected = bookmarkIsExist;
}

- (void)setHomeAppIsExist:(BOOL)homeAppIsExist
{
    _homeAppIsExist = homeAppIsExist;
    if (homeAppIsExist) {
    ...
    _btnHomeIcon.selected = homeAppIsExist;
}

/**
 *  设置是否存在标识
 *
 *  @param bookmark BOOL 书签是否存在
 *  @param homeApp  BOOL 首页App是否存在
 */
- (void)setExistFlagForBookmark:(BOOL)bookmark homeApp:(BOOL)homeApp
{
    BOOL isExistBookmark = bookmark;
    BOOL isExistHomeIcon = homeApp;
    if (isExistBookmark) {
        [_btnBookmark setTitle:LocalizedString(@"yichushuqian") forState:UIControlStateNormal];
        [_btnBookmark setTitle:LocalizedString(@"yichushuqian") forState:UIControlStateHighlighted];
        [_btnBookmark setTitle:LocalizedString(@"yichushuqian") forState:UIControlStateSelected];
