...
- (void)doAddMode
{
    ...
    if ([ADOMode isExistWithName:_viewReminderEdit.modelName sysRecommend:NO userId:[UserManager shareUserManager].currUser.uid]) {
        [SVProgressHUD showErrorWithStatus:LocalizedString(@"yicunzaitongmingmoshi_qinggenggaimoshiming")];
        return;
    }
...
- (void)doAddModeProgram
{
    ...
    if (modeProgram) {
        ...
        if (_modelProgramSelected.parent_pkid_server>0) {
            ...
            // 既然是添加，默认是启用状态
            modeProgram.on = YES;
            modeProgram.updateTime = [[NSDate date] timeIntervalSince1970];
            BOOL flag = [ADOModeProgram updateModel:modeProgram];
            
            // TODO:同步更新
            if (flag && modeProgram.pkid_server>0 && [SyncHelper shouldAutoSync] && [SyncHelper shouldSyncWithType:SyncDataTypeReminder]) {
                [[SyncHelper shareSync] syncUpdateArrModeProgram:@[modeProgram] completion:^{
        ...
        modeProgram.modePkidServer = [ADOMode queryModePkidServerWithModePkid:_modePkid];
        modeProgram.modelProgram = _modelProgramSelected;
        modeProgram.repeatMode = _viewReminderEdit.repeatMode;
        modeProgram.time = _viewReminderEdit.time;
        modeProgram.on = YES;
...
- (void)doEditModeProgram
{
    ...
    if (modeProgramExist) {
        ...
        if (_modelProgramSelected && _modelProgramSelected.parent_pkid_server>0) {
            ...
            ModelModeProgram *modeProgramWillDelete = [ADOModeProgram queryWithModePkid:modeProgramExist.modePkid time:time];
            BOOL flag = [ADOModeProgram deleteWithModePkid:modeProgramExist.modePkid time:time];
            
            // 记录同步删除项
            ModelSyncDelete *modelSyncDelete = nil;
            if (modeProgramWillDelete && modelSyncDelete.pkid_server>0) {
                modelSyncDelete = [ModelSyncDelete modelWithPkidServer:modelSyncDelete.pkid_server syncDataType:SyncDataTypeModeProgram userId:[UserManager shareUserManager].currUser.uid];
            ...
            _modelModeProgramEdit.updateTime = [[NSDate date] timeIntervalSince1970];
            ...
            // TODO:同步更新
            if (flag && [SyncHelper shouldAutoSync] && [SyncHelper shouldSyncWithType:SyncDataTypeReminder]) {
                if (modelSyncDelete.pkid>0) {
                    // TODO:同步删除
                    [[SyncHelper shareSync] syncDeleteModeWithArrSyncDelete:@[modelSyncDelete] completion:^{
                        if (_modelModeProgramEdit.pkid_server>0) {
                            [[SyncHelper shareSync] syncUpdateArrModeProgram:@[_modelModeProgramEdit] completion:^{
        ...
        // TODO:同步更新
        if (flag && _modelModeProgramEdit.pkid_server>0 && [SyncHelper shouldAutoSync] && [SyncHelper shouldSyncWithType:SyncDataTypeReminder]) {
            [[SyncHelper shareSync] syncUpdateArrModeProgram:@[_modelModeProgramEdit] completion:^{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIControllerProgramList *controllerProgramList = [UIControllerProgramList controllerFromXib];
