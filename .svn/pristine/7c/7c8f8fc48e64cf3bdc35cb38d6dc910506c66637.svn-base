...
{
    return [[DBHelper db] intForQuery:@"select pkid from tab_mode where pkid_server=? and lan=? limit 1", @(modePkidServer), [LocalizationUtil currLanguage]];
}

+ (NSInteger)queryModePkidServerWithModePkid:(NSInteger)modePkid
{
    return [[DBHelper db] intForQuery:@"select pkid_server from tab_mode where pkid=? and lan=? limit 1", @(modePkid), [LocalizationUtil currLanguage]];
}
...
+ (BOOL)deleteWithArrPkidServer:(NSArray *)arrPkidServer userId:(NSInteger)userId
{
    ...
    NSString *sql = [NSString stringWithFormat:@"delete from tab_mode where user_id=%ld and lan='%@' and pkid_server>0 and pkid_server not in (%@)", (long)userId, [LocalizationUtil currLanguage], pkidServers];
