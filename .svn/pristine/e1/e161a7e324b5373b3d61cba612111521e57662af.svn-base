...
- (void)viewDidLoad
{
    ...
    _imageViewIcon.image = [UIImage imageWithBundleFile:[NSString stringWithFormat:@"trans/cate_%@.png", _modelCate.name]];
    _labelCate.text = [LocalizationUtil getTextByKey:_modelCate.name];
    ...
    for (NSInteger i=0; i<_arrSubCate.count; i++) {
        ...
        viewHeader.labelTitle.text = [LocalizationUtil getTextByKey:modelCate.name];
...
//                             _tableView.alpha = 0;
                             ...
                             _labelPickerTitle.text = [LocalizationUtil getTextByKey:@"yuanyuyan"];
                             if (_btnDest.selected) {
                                 lan = _lanDes;
                                 _labelPickerTitle.text = [LocalizationUtil getTextByKey:@"mubiaoyuyan"];
                             }
                             NSInteger row = 0;
                             // 计算 选中 语言的行号
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView==_tableView) {
        ...
        if (!cell) {
            cell = [[UICellTrans alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"UICellTrans"];
        }
...
{
    NSString *lanName = [LocalizationUtil getTextByKey:_arrLan[row][@"lan"]];
